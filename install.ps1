# install.ps1 — Install WilowForClaude-itagents skills (companion to WilowForClaude-skills)
# Remote install: irm https://raw.githubusercontent.com/fransanda/WilowForClaude-itagents/main/install.ps1 | iex
...
if ($PSScriptRoot -and (Test-Path (Join-Path $PSScriptRoot "skills\itagentsreview\SKILL.md"))) {
    ...
    git clone --depth=1 --quiet https://github.com/fransanda/WilowForClaude-itagents.git $tempClone
    if ($LASTEXITCODE -ne 0) {
        Write-Host "Error: failed to clone WilowForClaude-itagents (network down or repo unavailable)." -ForegroundColor Red
...
Write-Host ""
Write-Host "Installing WilowForClaude-itagents skills..." -ForegroundColor Cyan
Write-Host ""
...
if ($installed.Count -eq 4) {
    ...
    Write-Host ""
    Write-Host "Note: requires WilowForClaude-skills installed first." -ForegroundColor Gray
    Write-Host ""
} else {
