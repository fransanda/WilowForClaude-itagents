...
- **Reality vs. intent (WIREFRAME.yaml)** — when the project has a `WIREFRAME.yaml` (generated by `/kickoff` / `/autonomy`), the testers use it as an **answer key**: they verify each CTA reaches its *declared* destination, that auth-gated pages can't be reached logged-out (catches "Get started" dumping you into Home with no login), that declared modal/sheet interactions work (e.g. a bottom sheet actually drags to dismiss), that `back` affordances exist where intended, and that forms behave as declared — required and conditional (`show_when`) fields, gated (`enabled_when`) CTAs, cross-field `rules`, and the declared error on invalid input. Plus universal heuristics it always applies — loading/empty/error states, focus trapping, keyboard/back-button behavior.
