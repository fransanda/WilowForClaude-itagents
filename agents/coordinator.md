...
3. Start the dev server (or use a configured staging URL), then deploy one `ui-tester` agent per role **in parallel** (they are read-only to code). Cap concurrency to ~4–6. **If `WIREFRAME.yaml` exists, pass it to each agent as the intent answer-key** so they check reality-vs-intent (declared flows, auth gating, component `dismiss` interactions, `back` intent, form `on_success`/`on_error`, declared states) — not just liveness. Also run the deterministic drift check (routes in code vs wireframe pages) and fold drift into the feedback.
